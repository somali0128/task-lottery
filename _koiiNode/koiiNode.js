...
class NamespaceWrapper {
  ...
  async processLotteryWinnerNumber(values) {
    // Sum all numbers in all hashes
    console.log('values is ', values);
    let totalSum = values.reduce((sum, item) => {
      // Correctly access the submission_value of each hash
      const numbersInHash = item.submission_value.match(/\d/g) || [];
      const sumOfNumbers = numbersInHash.reduce((acc, num) => acc + parseInt(num, 10), 0);
      return sum + sumOfNumbers;
    }, 0);
  
    // Convert the sum to a string to easily pick characters
    const sumStr = totalSum.toString();
  
    // Ensure the string is long enough to pick characters
    if (sumStr.length < 2) {
      throw new Error("The sum's string representation doesn't have enough characters.");
    }
  
    // Pick the first, second, last-1, and last characters
    const resultStr = `${sumStr[0]}${sumStr[sumStr.length - 1]}`;
  ...
  async validateAndVoteOnNodes(validate, round) {
    ...
    if (submissions == null) {
      ...
      // process the lottery winner number
      const winnerNum = await this.processLotteryWinnerNumber(values);
      ...
      for (let i = 0; i < size; i++) {
        ...
        if (candidatePublicKey == submitterPubkey) {
          ...
          try {
            ...
            isValid = await validate(values[i].submission_value, winnerNum, round);
